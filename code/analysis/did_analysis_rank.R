...
dataset <- st_read('./data/final_datasets/italy.geojson') |>
  mutate(year = as.factor(year),
         ranking = rank(number_of_innovations, ties.method="min"))

# Diff-in-diff analysis
formula_did <- as.formula(
  ranking ~ year + group + year*group + 
...
# Table
notes <- "Dependent variables: Rank(number of innovations) in municipality $i$, 
where higher is more innovative.  
...
knitr::opts_current$set(label = "did_analysis_rank")
modelsummary(
  list(model_lin,
       model_lin2,
       model_pois,
  ...
  gof_map = gm_did, 
  coef_map = cm_did,
  title = "Difference-in-difference Estimates of Unification on Innovation (Ranking)",
  add_rows = rows,
  coef_omit = "Intercept",
  out = "kableExtra",
  output = "latex")   |>
  ...
  kableExtra::save_kable(file="./tables/did_analysis_rank.tex")
