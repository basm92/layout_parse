...
# Table
notes <- "Dependent variables: Dummy(Number of innovations > 1) in municipality $i$. 
...
knitr::opts_current$set(label = "did_analysis_dummy")
modelsummary(
  list(model_lin,
       model_lin2,
       model_pois,
  ...
  gof_map = gm_did, 
  coef_map = cm_did,
  title = "Difference-in-difference Estimates of Unification on Indicator Innovation",
  add_rows = rows,
  coef_omit = "Intercept",
  out = "kableExtra",
  output = "latex")   |>
  ...
  kableExtra::save_kable(file="./tables/did_analysis_dummy.tex")
