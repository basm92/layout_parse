...
final <- final |>
  mutate(patents_together_verz_italy = if_else(is.na(patents_together_verz_italy), 0, patents_together_verz_italy),
         patents_together_verz_italy_dummy = if_else(patents_together_verz_italy > 0, 1, 0),
...
patents1858 <- feols(patents_together_verz_italy_dummy ~ allegiance_1861, 
                     data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                     vcov=~PRO_COM)
patents1858pois <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861, 
                          data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                          vcov=~PRO_COM)
patents1858cv <- feols(patents_together_verz_italy_dummy ~ allegiance_1861 + interpolated_population + area_of_intersection + abs_distance_to_border, 
                       data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                       vcov=~PRO_COM)
patents1858poiscv <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population, 
                            data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                            vcov=~PRO_COM)
patents1858cvfe <- feols(patents_together_verz_italy_dummy ~ allegiance_1861 + interpolated_population + area_of_intersection + abs_distance_to_border | year, 
                         data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                         vcov=~PRO_COM)
patents1858poiscvfe <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population | year, 
                              data = final |> filter(abs(running) < bw, is.element(year, 1855:1858)),
                              vcov=~PRO_COM)
...
patents1867 <- feols(patents_together_verz_italy_dummy ~ allegiance_1861, 
                     data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                     vcov=~PRO_COM)
patents1867pois <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861, 
                          data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                          vcov=~PRO_COM)
patents1867cv <- feols(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population, 
                       data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                       vcov=~PRO_COM)
patents1867poiscv <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population, 
                            data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                            vcov=~PRO_COM)
patents1867cvfe <- feols(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population | year, 
                         data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                         vcov=~PRO_COM)
patents1867poiscvfe <- fepois(patents_together_verz_italy_dummy ~ allegiance_1861 + area_of_intersection + abs_distance_to_border + interpolated_population | year, 
                              data = final |> filter(abs(running) < bw, is.element(year, 1860:1867)),
                              vcov=~PRO_COM)
...
# Table notes
n <- "Table reports estimates of an indicator whether Patents > 0 in municipalities in Lombardy relative to Veneto.  
