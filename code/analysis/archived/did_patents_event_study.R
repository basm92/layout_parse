...
dataset <- read_sf('./data/italy_austrian_piedmontese_patents.geojson') |>
  mutate(post = if_else(year > 1859 & ds > ymd("1859-06-01"), 1, 0),
         ename = if_else(group == "Lombardia", year - 1859, 0),
...
formula_did <- as.formula(
  patents ~ i(group, i.ename, ref="Veneto", ref2=0)
)
...
fe <- as.formula(
  patents ~ i(group, i.ename, ref="Veneto", ref2=0) | NUTS_ID
)
...
model1 <- feols(formula_did, data = dataset, vcov='hc1')
model2 <- feols(formula_did_controls, data = dataset, vcov='hc1')
model3 <- feols(fe, data = dataset, vcov ='hc1')
model4 <- feols(fe_controls, data = dataset, vcov='hc1')

# Table
notes <- "Dependent variables: Number of patents in municipality $i$. The estimates are computed 
using the Borusyak, Jaravel and Spiess (2021) estimator. The coefficients of interest are 
the Treated x Pre-Post Treatment Year variables. The reference year is 1859. 
...
knitr::opts_current$set(label = "did_patents_event_study")
modelsummary(
  ...
  kableExtra::save_kable(file="./tables/did_patents_event_study.tex")
