...
formula_did <- as.formula(
  patents ~ i(group, ref="Veneto")*post
)
...
fe <- formula_did <- as.formula(
  patents ~ i(group, ref="Veneto")*post | NUTS_ID
)
...
# Table
notes <- "Dependent variables: Number of patents in municipality $i$. 
...
knitr::opts_current$set(label = "did_patents_pre_post")
modelsummary(
  ...
  kableExtra::save_kable(file="./tables/did_patents_pre_post.tex")
